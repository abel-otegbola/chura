...
import { HamburgerMenu, Home, type IconProps } from "@solar-icons/react";
...
function Sidebar() {
    ...
    const generalLinks: Link[] = [
        { id: 0, label: "Dashboard", icon: <Home size={20}/>, link: "/" },
        { id: 1, label: "Predictor", icon: <RobotIcon size={20}/>, link: "/predictor" },
    ]
    ...
    return (
        <div ref={modalRef} className="md:sticky top-0 left-0 h-screen md:w-[280px] w-0 md:p-4">
            ...
            <div className={`flex flex-col md:w-full w-[280px] md:h-full h-[100vh] md:rounded-[20px] md:sticky dark:bg-[#151515] fixed md:top-0 top-0 py-4 px-4 right-0 overflow-y-auto overflow-x-hidden z-[5] transition-all duration-700 ${open ? "translate-x-[0px] opacity-[1]": "translate-x-[400px] md:translate-x-[0px] md:opacity-[1] opacity-[0]"}`}>  
                ...
                <div className="flex flex-col gap-1">
                    ...
                    generalLinks.map(link => {
                            return (
                            <Link key={link.id} onClick={() => setOpen(false)} to={ link.link} className={`relative flex items-center justify-between px-4 py-1 md:rounded-[10px] duration-300 font-medium ${pathname.includes(link.link) ? "bg-primary/[0.12]" : " hover:bg-gray-500/[0.06] border-l-3 border-transparent hover:dark:bg-gray-500/[0.09]"}`}>
